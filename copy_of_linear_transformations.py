...
Original file is located at
    https://colab.research.google.com/drive/1K6MUXdx5MRSnKE6URQhf_A7LZfqTREcv
...
def stretch(image,a,b):
  stretch_matrix = np.array([[a, 0],
                             [0, b]])
  stretched_image = np.dot(stretch_matrix, image)
  return stretched_image

showplot(stretch(cougar, 1/2, 6/5))
...
def shear(image,a,b):
  shear_matrix = np.array([[1, a],
                           [b, 1]])
  sheared_image = np.dot(shear_matrix, image)
...
def reflect(image,a,b):
  reflect_matrix = np.array([[(a**2)-(b**2), 2*a*b],
                           [2*a*b, (b**2)-(a**2)]]) / ((a**2)+(b**2))
  reflect_image = np.dot(reflect_matrix, image)
...
def rotate(image,theta):
  ...
  rotated_image = np.dot(rotate_matrix, image)
...
step_1 = stretch(iden, 1, 2)

step_2 = rotate(iden, -np.pi/4)

step_3 = reflect(iden, -3, 2)

step_4 = stretch(iden, 1/2, 1)

comp_matrix= np.dot(step_4, np.dot(step_3, np.dot(step_2, step_1)))
...
plot_transformation(comp_matrix)
...
def area(A):
  ...
  matrix = np.column_stack((T_e1, T_e2))
...
det_A = np.abs(np.linalg.det(ellipse_matrix))

ellipse_area= det_A * np.pi
